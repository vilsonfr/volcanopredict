...
- [x] 7.2 Criar workflow do GitHub Actions rodando build, `go vet`, testes Go completos e build do frontend; verificar que o workflow passa verde na primeira execução — `.github/workflows/ci.yml`, com job de backend (build, vet, suíte curta, suíte completa com testcontainers) e job de frontend (`npm ci` + build). Os mesmos comandos foram executados localmente e passaram; o verde do próprio Actions só é observável depois do push
