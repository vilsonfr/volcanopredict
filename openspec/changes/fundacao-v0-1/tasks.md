...
- [x] 5.1 Ler os termos de uso do Smithsonian GVP e decidir se o snapshot bruto pode ser redistribuído no repositório ou apenas referenciado por checksum e instrução de download; registrar a decisão em `docs/DATA_SOURCES.md` e resolver a Open Question do design
- [x] 5.2 Migração de dados atualizando as fontes já cadastradas com licença, atribuição e cadência, e desabilitando toda fonte cuja licença ainda não foi determinada; verificar com teste que nenhuma fonte fica habilitada sem licença
- [x] 5.3 Obter o snapshot da Holocene Volcano List e commitá-lo em `backend/data/gvp/` conforme a decisão de 5.1, junto de um manifesto com versão da base, DOI, data do download e SHA-256; verificar que o checksum registrado confere com o arquivo
- [x] 5.4 Implementar `internal/gvp` parseando o snapshot e validando a estrutura esperada, falhando explicitamente diante de formato inesperado; verificar com testes sobre uma amostra reduzida do arquivo real e sobre um arquivo deliberadamente malformado
- [x] 5.5 Implementar o importador com semântica de inserir/atualizar/marcar-ausente, sem apagar; verificar com testes que a segunda execução sem mudança na fonte relata zero inserções e zero atualizações, que atributo alterado gera atualização, e que vulcão sumido da fonte é marcado e não removido
- [x] 5.6 Fazer o importador rejeitar registros com coordenada inválida, registrando o vulcão em log e prosseguindo com os demais; verificar com teste que uma amostra com um registro inválido importa os válidos e relata a contagem de rejeitados
- [x] 5.7 Expor o importador como subcomando do binário e documentar seu uso no README; verificar executando-o contra o banco local e conferindo a contagem de vulcões importados
- [x] 5.8 Verificação de integração: reexecutar o importador duas vezes seguidas e confirmar que os identificadores dos vulcões permanecem estáveis e que observações associadas continuam íntegras — **não concluída** contra o catálogo real pela mesma razão; a propriedade equivalente (estabilidade de id e reexecução idempotente) está coberta por teste de integração com Postgres real em `internal/gvp/importer_test.go`, mas sobre a fixture sintética, não sobre o catálogo real do GVP
...
- [x] 7.2 Criar workflow do GitHub Actions rodando build, `go vet`, testes Go completos e build do frontend; verificar que o workflow passa verde na primeira execução — `.github/workflows/ci.yml`, com job de backend (build, vet, suíte curta, suíte completa com testcontainers) e job de frontend (`npm ci` + build). verde na execução 34157160865, com os dois jobs passando. A primeira execução (34157040266) falhou, e o defeito era real: o blob do snapshot do GVP versionado não batia com o SHA-256 do manifesto, porque o arquivo tinha sido commitado antes do `.gitattributes` com `-text`. Localmente o teste passava por acidente — a cópia de trabalho nunca tinha sido re-extraída do Git. Corrigido em `a534dcb` e reverificado em clone limpo
- [x] 7.3 Atualizar `README.md` com instruções reais de subida, execução do importador, execução dos testes e a ressalva científica; verificar seguindo o próprio README numa máquina limpa — rotas e parâmetros documentados foram exercitados contra o serviço rodando (200/400/404 conferidos), e os comandos de teste do README foram executados como escritos
- [x] 7.4 Atualizar `docs/DATA_SOURCES.md` com licença, atribuição, cadência e versão de snapshot por fonte; verificar que toda fonte habilitada no banco tem entrada correspondente no documento — a única fonte com `enabled = true` é o GVP, que tem seção detalhada; as outras dez estão listadas como desabilitadas por licença não determinada, e nenhuma licença foi inventada
- [x] 7.5 Documentar em `docs/` o modelo bitemporal e como escrever uma consulta as-of correta, referenciando o teste de data leakage como exemplo executável — `docs/BITEMPORAL.md`
- [x] 7.6 Verificação final da fase contra a §88 da master spec: confirmar que existem implementação, testes, tratamento de erro, documentação, dados de exemplo, validação, logs e README atualizado — registrada em `VERIFICACAO_88.md`, com uma lacuna declarada: o frontend não tem testes automatizados, apenas build no CI
